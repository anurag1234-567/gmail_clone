...
const SearchPage = () =>{
  ...
  useEffect(()=>{
    ...
    const fetchdata = async()=>{
      try{
        const res = await axios.get(`http://localhost:8000/search/${query}/${pageNumber}`);
  ...
  return(
    ...
    { !isLoading && 
      <>
        ...
        {
          data && data.length === 0 ? <p className='empty-message'>You don't have any mail!</p>
          : (
