...
const Starred = () =>{
  ...
  useEffect(()=>{
    ...
    const fetchdata = async()=>{
      try{
        const res = await axios.get(`http://localhost:8000/emails/starred/${pageNumber}`);
  ...
  return(
    ...
    { !isLoading && (<>
      ...
      {
        data && data.length === 0 ? <p className='empty-message'>No Starred messages. Stars let you give messages a special status to make them easier to find. To Star a message, click on the star outside beside any message or conversation.</p>
        : (
